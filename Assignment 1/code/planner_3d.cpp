...
class a_star
{
    ...
    void compute_path()
    {
        ...
        while(!(open.empty()))
        {
            ...
            for(int dir = 0; dir < NUMOFDIRS; dir++)
            {
                ...
                thread_list.emplace_back(&a_star::explore_succ,this,curr,new_loc,dir);
            }
    ...
    void explore_succ(Node* curr, Point new_loc, int dir)
    {
        if (new_loc.x >= 0 && new_loc.x < x_size && new_loc.y >= 0 && new_loc.y < y_size) //Within Bounds
        {
            if (closed.count(new_loc) == 0) //Not in closed list
            {
                ...
                if ((t_c >= 0) && (t_c < collision_thresh))  //if free
                {
                    ...
                    const std::lock_guard<std::mutex> lock2(my_map_mutex);
                    if (it==my_map
                    .end()) 
                    {
                        my_map[new_loc] = temp;
