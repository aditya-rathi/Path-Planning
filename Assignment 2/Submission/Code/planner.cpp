...
double* random_config(double* map, int x_size, int y_size, int numofDOFs, double* armgoal_anglesV_rad)
{
	std::uniform_real_distribution<double> num_gen(0,2*PI);
	std::uniform_real_distribution<double> goal_gen(0,1);
	double* config = (double*) malloc(numofDOFs*sizeof(double));

	if (goal_gen(eng) < 0.15)
	{
		std::uniform_real_distribution<double> goal_region(-0.06,0.06);
		for (int i = 0; i<numofDOFs; i++)
		{
			config[i] = armgoal_anglesV_rad[i] + goal_region(eng); 
		}
		if (IsValidArmConfiguration(config,numofDOFs,map,x_size,y_size)) return config;
	}
...
std::vector<int> nearest_nhb_star(const std::vector<double*>& samples, double* curr, int numofDOFs, double eps)
{
	std::priority_queue<std::pair<double,int>,std::vector<std::pair<double,int>>,std::greater<std::pair<double,int>>> pq;
	double hyp_vol;
		switch(numofDOFs)
	...
	double r = std::min(std::pow((1/hyp_vol)*(std::log(samples.size())/samples.size()),1.0/numofDOFs),eps);
...
bool isValidInterpolation(double* a, double* b, int numofDOFs, double* map, int x_size, int y_size)
{
	double unit_vec[numofDOFs];
	int numofsamples = std::max((int) euc_dist(a,b,numofDOFs)*10,10);
	for (int i = 0; i < numofsamples; i++){
        for(int j = 0; j < numofDOFs; j++){
            unit_vec[j] = a[j] + ((double)(i)/(numofsamples-1))*(b[j] - a[j]);
        }
        if(!IsValidArmConfiguration(unit_vec, numofDOFs, map, x_size, y_size)) return false;
    }
	return true;
}

bool isGoalVisible(double* a, double* goal, int numofDOFs, double* map, int x_size, int y_size)
{
	if (euc_dist(a,goal,numofDOFs) < 1) return isValidInterpolation(a, goal, numofDOFs, map, x_size, y_size);
	return false;
}
...
void rrt(double* map, int x_size, int y_size, double* armstart_anglesV_rad, double* armgoal_anglesV_rad, int numofDOFs, double*** plan, int* planlength)
{
	...
	*planlength = 0;
	...
	while(nodes_added<15000 && !goalVisible)
	{
		double* curr = random_config(map, x_size, y_size, numofDOFs, armgoal_anglesV_rad);
		...
		if(!isValidInterpolation(node_list[nearest_nhb_ind],data.second,numofDOFs,map,x_size,y_size)) continue;
		
		node_list.push_back(data.second);
		parent_list.push_back(nearest_nhb_ind);
		nodes_added++;
		nhb_coord = data.second;
		goalVisible = isGoalVisible(data.second, armgoal_anglesV_rad, numofDOFs, map, x_size, y_size);
	}
...
void rrt_connect(double* map, int x_size, int y_size, double* armstart_anglesV_rad, double* armgoal_anglesV_rad, int numofDOFs, double*** plan, int* planlength)
{
	...
	*planlength = 0;
	...
	while(!trees_connected && nodes_added<15000)
	{
		...
		double* sample = random_config(map, x_size, y_size, numofDOFs, armgoal_anglesV_rad);
		int nearest_nhb_ind;
		...
		if (which_tree) 
		{
			if(!isValidInterpolation(fwd_node_list[nearest_nhb_ind],data.second,numofDOFs,map,x_size,y_size)) continue;
		}
		else
		{
			if(!isValidInterpolation(back_node_list[nearest_nhb_ind],data.second,numofDOFs,map,x_size,y_size)) continue;
		}
...

{
	...
	*planlength = 0;
	...
	while(!goalvisible && nodes_added<15000)
	{
		//Generate random sample
		double* curr = random_config(map, x_size, y_size, numofDOFs, armgoal_anglesV_rad);
		int nearest_nhb_ind = nearest_nhb(node_list,curr,numofDOFs);
		...
		// Rewiring
		std::vector<int> neighbors = nearest_nhb_star(node_list,data.second,numofDOFs,eps);
		// if (!neighbors.empty()) cout<<neighbors.size()<<endl;
		for (int i = 0; i<neighbors.size(); i++)
		{
			if(!isValidInterpolation(node_list[neighbors[i]],data.second,numofDOFs,map,x_size,y_size)) continue;
...
void prm(double* map, int x_size, int y_size, double* armstart_anglesV_rad, double* armgoal_anglesV_rad, int numofDOFs, double*** plan, int* planlength)
{
	...
	*planlength = 0;
	...
	{
		samples.push_back(random_config(map,x_size,y_size,numofDOFs, armgoal_anglesV_rad));
		// uf_array.push_back(i);
	}

	
	// int nearest_nhb_ind = nearest_nhb(samples,armgoal_anglesV_rad,numofDOFs);
	// bool goalvisible = isGoalVisible(samples[nearest_nhb_ind],armgoal_anglesV_rad,numofDOFs,map,x_size,y_size);
	// double* new_config;
	// while(!goalvisible)
	// {
	// 	new_config = random_config(map,x_size,y_size,numofDOFs, armgoal_anglesV_rad);
	// 	samples.push_back(new_config);
	// 	goalvisible = isGoalVisible(new_config,armgoal_anglesV_rad,numofDOFs,map,x_size,y_size);
	// }

	// nearest_nhb_ind = nearest_nhb(samples,armstart_anglesV_rad,numofDOFs);
	// bool startvisible = isGoalVisible(samples[nearest_nhb_ind],armstart_anglesV_rad,numofDOFs,map,x_size,y_size);
	// while(!startvisible)
	// {
	// 	new_config = random_config(map,x_size,y_size,numofDOFs, armgoal_anglesV_rad);
	// 	samples.push_back(new_config);
	// 	goalvisible = isGoalVisible(new_config,armstart_anglesV_rad,numofDOFs,map,x_size,y_size);
	// }

	// numsamples = samples.size();
	// cout<<numsamples<<endl;

	std::vector<std::vector<double>> edges(numsamples+2,std::vector<double>(numsamples+2,0));
	...
	for(int i=0;i<numsamples;i++)
	{
		...
		for (int j = 0;j<temp.size();j++)
		{
			...
			int nhb_ind = temp[j];
			if (edges[i][nhb_ind] == 0 && isValidInterpolation(samples[i],samples[nhb_ind],numofDOFs,map,x_size,y_size)) 
			{
				edges[i][nhb_ind] = euc_dist(samples[i],samples[nhb_ind],numofDOFs);
				// Since our graph is undirected
				edges[nhb_ind][i] = euc_dist(samples[i],samples[nhb_ind],numofDOFs);
			}
	...
	k = 3; //Check 5 closest neighbors when connecting start and goal
	...
	for (int i=0; i<k; i++)
	{
		...
		edges[numsamples][start_nhb[i]] = 1;
		edges[start_nhb[i]][numsamples] = 1;
	...
	std::vector<double> distances(numsamples+2,std::numeric_limits<double>::max());
	...
	for(int i=0;i<numsamples+2;i++)
	{
		...
		visited[curr] = true;
		if (curr == numsamples+1) break;

		for (int j = 0; j<numsamples+2;j++)
		{
			double edgecost = edges[curr][j];
